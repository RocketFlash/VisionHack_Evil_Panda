...
translationFactorCalibration = 400.0
...
inputPhoto = cv2.imread('/Users/YagfarovRauf/Desktop/zzzz.jpg')
...
if height > width:
    translationFactor = translationFactorCalibration / height
else:
    translationFactor = translationFactorCalibration / width
# print translationFactor

# resize the image so the longest edge is 1200 pixels, keeping the same aspect ratio
resizedInputPhoto = cv2.resize(inputPhoto, None, fx=translationFactor, fy=translationFactor,
...
for key, val in possibleLines.items():
    x = val[0]
    w = val[2]

    midPt = (x + x + w) / 2
        # print "MIDPT for {} is {}".format(key, midPt)
    if ((median - (translationFactorCalibration / 10)) <= midPt <= (
                median + (translationFactorCalibration / 10))):
        crossLineContourIndexes.append(key)

if len(crossLineContourIndexes) >= 2:
